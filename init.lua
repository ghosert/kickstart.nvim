...
-- Save undo history, jiawzhang updated to false
...
-- jiawzhang added for 'gruvbox' colorscheme below.
...
vim.keymap.set('n', '[d', function()
  vim.diagnostic.goto_prev { count = 1 }
end, { desc = 'Go to previous [D]iagnostic message' })
vim.keymap.set('n', ']d', function()
  vim.diagnostic.goto_next { count = 1 }
...
-- NOTE: Using <C-q> instead of <Esc><Esc> to avoid conflict with timeoutlen=0 to exit terminal mode, 't' below means terminal
-- Press <C-q> in terminal mode to switch back to normal mode from terminal mode, so that you can copy/paste in terminal
vim.keymap.set('t', '<C-q>', '<C-\\><C-n>', { desc = 'Exit terminal mode' })

--Fix space key delay in terminal mode by disabling timeoutlen
vim.api.nvim_create_autocmd('TermOpen', {
  desc = 'Disable leader key timeout in terminal mode',
  group = vim.api.nvim_create_augroup('terminal-timeoutlen', { clear = true }),
  callback = function()
...
require('lazy').setup({
  ...
  { -- Autoformat jiawzhang: auto format when typing :w to save lua file using 'stylua' LSP formatter.
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        local disable_filetypes = { c = true, cpp = true, javascript = true } -- NOTE: disabled auto format on save for javascript, since we have too many existing js files.
    ...
    dependencies = {
      -- Snippet Engine & its associated nvim-cmp source
      {
        ...
        dependencies = {
          ...
          { -- jiawzhang: uncommented this for auto complete snippet with vscode style.
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      --  - yi' or yinq - [Y]ank [I]nside [N]ext [']quote
    ...
    config = function()
      ...
      local bookmarks_section = {
        ...
        val = {
          ...
          dashboard.button('N', 'neovim tips', ':e ~/VimProject/note-taking/neovim.md <CR>'),
