...
M._defaults = {
  ...
  ---@type avante.Mode
  mode = 'legacy', -- jiawzhang NOTE: change default mode to legacy
  ...
  provider = 'claude', -- jiawzhang NOTE: Only recommend using 'claude', 'ollama' for local
...
local function get_config_dir_path()
  return Utils.join_paths(vim.fn.expand '~', '.config', 'avante.nvim')
end
local function get_config_file_path()
  return Utils.join_paths(get_config_dir_path(), 'config.json')
end
...
function M.save_last_model(model_name, provider_name)
  ...
  if file then
    file:write(vim.json.encode { last_model = model_name, last_provider = provider_name, provider_model = provider_model })
    file:close()
  end
end
  ...
  if file then
    ...
    if data.last_provider then
      ...
      if not provider then
        Utils.warn('Provider ' .. data.last_provider .. ' is no longer a valid provider, falling back to default configuration')
        os.remove(storage_path)
        return
      end
...
local function apply_model_selection(config, model_name, provider_name)
  ...
  if config.acp_providers[current_provider_name] then
    return
  end
  ...
  if target_provider_name ~= current_provider_name or model_name ~= current_model_name then
    ...
    if not target_provider.model_names then
      target_provider.model_names = {}
    end
  ...
  for k, v in pairs(opts) do
    ...
    if k == 'vendors' and v ~= nil then
      for k2, v2 in pairs(v) do
        ...
        if type(v2) == 'table' and v2.__inherited_from ~= nil and M._defaults.providers[v2.__inherited_from] ~= nil then
...
require('which-key').add {
  { '<leader>v', group = 'A[v]ante' }, -- jiawzhang NOTE: add for avante.nvim
  {
    mode = { 'n', 'v' },
    {
      '<leader>vL',
